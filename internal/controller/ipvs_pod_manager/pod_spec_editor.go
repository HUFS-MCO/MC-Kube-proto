...
// PodSpecEditor handles Pod specification editing operations
type PodSpecEditor struct {
	Client   client.Client
	MinMilli int64
}

// NewPodSpecEditor creates a new PodSpecEditor instance
func NewPodSpecEditor(client client.Client, minMilli int64) *PodSpecEditor {
	return &PodSpecEditor{
		Client:   client,
		MinMilli: minMilli,
	}
}
...
func (pse *PodSpecEditor) DegradePodRequests(ctx context.Context, pod *corev1.Pod, ratio float64) error {
	...
	for _, c := range pod.Spec.Containers {
		...
		if newMilli < pse.MinMilli {
			newMilli = pse.MinMilli
		}
	...
	if err := pse.Client.SubResource("resize").Patch(ctx, pod, client.RawPatch(types.MergePatchType, patchBytes)); err != nil {
		return err
	}
	return nil
}
...
func HasActionableInTier(pods []*corev1.Pod, rtData map[string]RealTimeData, tier string, minMilli int64) bool {
